...
public class SliderGame4x4 extends JFrame implements ActionListener {
	...
	private int emptyIndex = 15; //Variable will track the empty spot
        private boolean check[] = new boolean[15];
	
        public static void main(String[] args) {

            new SliderGame4x4();      //Run constructor for class
	}
        
	public SliderGame4x4()
	{
		...
		gameboardPanel.setLayout(new GridLayout(4,4,5,5)); // 4x4 grid with 5 pixel padding										   // vert/horz dividers
		...
		{
                    int placeHolder = scrambler.nextInt(15);
			...
			gameboardPanel.add(buttons[i]);       //Add buttons to the grid layout of 
                ...
                buttons[15].setSize(100,100);
                //buttons[8] = buttons[emptyIndex];
		int colourIndex = 0;
                buttons[15].setBackground(colours[colourIndex]);
			buttons[15].setForeground(Color.blue);   //Text colour
			buttons[15].setFont(f);
			buttons[15].addActionListener(this);   //Set up ActionListener on each button
			gameboardPanel.add(buttons[15]);       //Add buttons to the grid layout of 
                buttons[15].setVisible(false);  //Will show the black background without a
						//visible button here
	...
	{
                if((emptyIndex+1) % 4 == 0){
                    if(emptyIndex-4==btnIndex||emptyIndex+4==btnIndex||emptyIndex-1==btnIndex){
                        setForSwap(btnIndex);                
                    }
                }
                    ...
                    if(emptyIndex-1==btnIndex||emptyIndex+1==btnIndex||emptyIndex-4==btnIndex||emptyIndex+4==btnIndex){
                        setForSwap(btnIndex);
