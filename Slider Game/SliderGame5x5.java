...
public class SliderGame5x5 extends JFrame implements ActionListener {

        private JFrame myFrame;
        private JPanel gameboardPanel; //create a panel to add to JFRAME
	private JButton[] buttons = new JButton[25];  //Make room for 16 button objects
        public int numMoves = 0;
	private int emptyIndex = 24; //Variable will track the empty spot
        private boolean check[] = new boolean[24];
	
        public static void main(String[] args) {

            new SliderGame5x5();      //Run constructor for class
	}
        
	public SliderGame5x5()
	{
		...
		gameboardPanel.setLayout(new GridLayout(5,5,5,5)); // 4x4 grid with 5 pixel padding										   // vert/horz dividers
		...
		{
                    int placeHolder = scrambler.nextInt(24);
			...
			gameboardPanel.add(buttons[i]);       //Add buttons to the grid layout of 
                ...
                buttons[24] = new JButton(""+ 24);
                buttons[24].setSize(100,100);
                //buttons[8] = buttons[emptyIndex];
		int colourIndex = 0;
                buttons[24].setBackground(colours[colourIndex]);
			buttons[24].setForeground(Color.blue);   //Text colour
			buttons[24].setFont(f);
			buttons[24].addActionListener(this);   //Set up ActionListener on each button
			gameboardPanel.add(buttons[24]);       //Add buttons to the grid layout of 
                buttons[24].setVisible(false);  //Will show the black background without a
						//visible button here
	...
	public void actionPerformed(ActionEvent e) {
		...
		for(int i=0;i<buttons.length;i++)
		{
			buttons[i] = (int)(1+25*Math.random());
                        swapPieces(i);
		}            
            }        
	...
	{
                if((emptyIndex+1) % 5 == 0){
                    if(emptyIndex-5==btnIndex||emptyIndex+5==btnIndex||emptyIndex-1==btnIndex){
                        setForSwap(btnIndex);
                    }
                }
                
                else if(emptyIndex % 5 == 0){
                    if(emptyIndex-5==btnIndex||emptyIndex+5==btnIndex||emptyIndex+1==btnIndex){
                        setForSwap(btnIndex);                
                    }
                }
                    ...
                    if(emptyIndex-1==btnIndex||emptyIndex+1==btnIndex||emptyIndex-5==btnIndex||emptyIndex+5==btnIndex){
                        setForSwap(btnIndex);          
                    }
                }
